...
public class UsagePresenter implements UsageContract.Presenter {
    ...
    public void retrieveUsageStats() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            usageStats = usageStatsManager.queryAndAggregateUsageStats(getStartTime(), System.currentTimeMillis());
        }
    ...
    private boolean checkForPermission(Context context) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
            AppOpsManager appOps = (AppOpsManager) context.getSystemService(Context.APP_OPS_SERVICE);
             mode = appOps.checkOpNoThrow(OPSTR_GET_USAGE_STATS, myUid(), context.getPackageName());
        }
    ...
    private UsageStatsWrapper fromUsageStat(String packageName) throws IllegalArgumentException {
        try {
            ...
            Log.e("err", "Errror: " + e.getMessage());
            Log.e("erro", "stacktrace: " + e.getStackTrace());
